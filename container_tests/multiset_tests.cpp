...
int	set_tester()
{
...

	{
		...
		iterator ret0 = set.insert(4);
		iterator ret1 = set.insert(8);
		bool ok = (*ret0 == 4) && (*ret1 == 8);
		cout << (ok ? "-> success" : "-> fail") << endl;
		iterator it = set.insert(ret1, 9);
		cout << (*it == 9 ? "-> success" : "-> fail") << endl;
	...
	set.erase(19);
	c_info(set);
	cout << "-> iterator parameter: erasing 1st 4\n";
	set.erase(ft::fwd(set.begin(), 3));
	c_info(set);
	cout << "-> iterator range: erasing [2nd 4, 34[\n";
	set.erase(ft::fwd(set.begin(), 4), ft::fwd(set.end(), -2));
	c_info(set);
	...
	if (high != set.end())
		cout << *high << endl;
	cout << "-> equal_range(2) = " << std::endl;
	std::pair<iterator, iterator> range = set.equal_range(2);
	C bis(range.first, range.second);
	c_info(bis);
	cout << "-> equal_range(76) = " << std::endl;
...
int main()
{
	...
	std::ofstream real("./diffs/real_multiset.txt");
	std::cout.rdbuf(real.rdbuf());
	set_tester<std::multiset<int, std::less<int>, std::allocator<int> > >();

	std::ofstream mine("./diffs/my_multiset.txt");
	std::cout.rdbuf(mine.rdbuf());
	set_tester<ft::multiset<int, std::less<int>, ft::allocator<int> > >();
